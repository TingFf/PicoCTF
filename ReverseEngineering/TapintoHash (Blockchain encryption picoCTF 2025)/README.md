## Comment  
1. Similar to Quantum scrambler, utilizing chatGPT to help me solve the challenge.

## Challenge Overview  
**Difficulty:** Medium  
**Description:** Write a script to RE the encryption algorithm.  
## Tools Used  
NIL  
...
def encrypt(plaintext, inner_txt, key):
    ...
    return ciphertext
```
3. To RE, we must start from the back.
   - XOR each block again
   - Remove the padding
   - inner_txt = Flag
```
def unpad(data):
...
## Stuff Learned  
1. To RE must start from the back and work it way back.  
